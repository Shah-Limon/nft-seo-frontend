...
const PaymentSetting = () => {
  ...
  useEffect(() => {
    fetch(`https://nft-seo-server.vercel.app/payment-accounts`)
      .then((res) => res.json())
      .then((info) => setPayments(info));
  }, []);

  useEffect(() => {
    fetch(`https://nft-seo-server.vercel.app/cash-on-delivery`)
      .then((res) => res.json())
  ...
  const handleCashOnDeliveryEnable = (event) => {
    ...
    const url = `https://nft-seo-server.vercel.app/cash-on-delivery`;
    fetch(url, {
      method: "POST",
      headers: {
  ...
  const handleAddNewPaymentAccount = (event) => {
    ...
    const url = `https://nft-seo-server.vercel.app/add-payment-account`;
    fetch(url, {
      method: "POST",
      headers: {
        "content-type": "application/json",
