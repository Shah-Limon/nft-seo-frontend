...
const HomeSectionTwoEdit = () => {
  ...
  const handleSectionTwoForRecentProductsEdit = (event) => {
    event.preventDefault();
    const setSection = event.target.setSection.value;
    const setSectionForRecentProducts = { setSection };

    const url = `http://localhost:5000/section-two-edit-recent-products/${id}`;
    fetch(url, {
      method: "PUT",
      headers: {
        "content-type": "application/json",
  ...
  return (
    <div>
        <form onSubmit={handleSectionTwoEdit}>
        ...
        <form onSubmit={handleSectionTwoForRecentProductsEdit}>
...
export default HomeSectionTwoEdit;
