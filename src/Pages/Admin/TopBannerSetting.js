...
const TopBannerSetting = () => {
  ...
  useEffect(() => {
    fetch(`https://nft-seo-server.vercel.app/top-banner`)
      .then((res) => res.json())
  ...
  const handleTopBanner = (event) => {
    ...
    const url = `https://nft-seo-server.vercel.app/top-banner`;
    fetch(url, {
      method: "POST",
      headers: {
  ...
  return (
    <div className="container mx-auto">
      {topBanners.filter((topBanner) => topBanner.bannerTitle).length ===
        0 && (
        <form onSubmit={handleTopBanner}>
          <span className="label-text">Your Banner Title</span><br></br>
          <input required type="text" placeholder="Your Banner Title" name="bannerTitle"></input>
          <br></br>
          <span className="label-text">Your Banner Text</span><br></br>
          <input type="text" name="bannerText" placeholder="Type your Banner Text"></input>
          <br></br>
          <span className="label-text">Your Banner banner Button Text </span><br></br>
          <input type="text" name="bannerButtonText" placeholder="Type Banner banner Button Text Link"></input>
          <br></br>
          <span className="label-text">Your Banner banner Button Text Link</span><br></br>
          <input type="text" name="bannerButtonTextLink" placeholder="Type Banner banner Button Text Link"></input>
          <br></br>
          <span className="label-text">Banner Image Link</span><br></br>
          <input type="text" name="bannerImage" placeholder="Type Banner Image link"></input>
          <br></br>
          <span className="label-text">Banner Background Image</span><br></br>
          <input type="text" name="bannerBackgroundImage" placeholder="Banner Background Image"></input>
          <br></br>
      ...
      )}
      {
        ...
        <>
        {
            topBanners.map(topBanner=> 
                <Link className="btn" to={`/admin/topbanner/${topBanner._id}`}>Edit This Banner</Link>
                )
        }
        </>
      }
