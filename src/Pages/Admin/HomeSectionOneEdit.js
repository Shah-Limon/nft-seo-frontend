...
const HomeSectionOneEdit = () => {
  ...
  const handleSectionOneForRecentProductsEdit = (event) => {
    event.preventDefault();
    const setSection = event.target.setSection.value;
    const setSectionForRecentProducts = { setSection };

    const url = `http://localhost:5000/section-one-edit-recent-products/${id}`;
    fetch(url, {
      method: "PUT",
      headers: {
        "content-type": "application/json",
  ...
  return (
    <div>
        <form onSubmit={handleSectionOneEdit}>
        ...
        <form onSubmit={handleSectionOneForRecentProductsEdit}>
...
export default HomeSectionOneEdit;
