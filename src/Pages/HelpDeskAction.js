...
const HelpDeskAction = () => {
  ...
  useEffect(() => {
    fetch(`https://nft-seo-server.vercel.app/ticket/${id}`)
      .then((res) => res.json())
      .then((info) => setTicket(info));
  }, [id]);

  useEffect(() => {
    fetch(`https://nft-seo-server.vercel.app/reply-tickets`)
      .then((res) => res.json())
  ...
  const HandleTicketReply = (event) => {
    ...
    const url = `https://nft-seo-server.vercel.app/add-ticket-reply`;
    fetch(url, {
      method: "POST",
      headers: {
  ...
  const HandleTicketStatus = (event) => {
    ...
    const url = `https://nft-seo-server.vercel.app/ticket/${id}`;
    fetch(url, {
      method: "PUT",
      headers: {
        "content-type": "application/json",
